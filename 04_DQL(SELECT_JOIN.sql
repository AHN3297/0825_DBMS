...
---------------------------------------------------------------------
   ...
   * 
   * 테이블간의 JOIND시 일치하지않는 행도 포하시켜서 ResultSet반환
   * 단, 반드시 LEFT / FIRHT를 지정해줘야함 !(기준 테이블을 선택해야함)
   */
  
  -- EMPLOYEE테이블에 존재하는 "모든"사원들의 사원명, 부서명
  -- INNER JOIN
  SELECT
         EMPLOYEE
       , DEPT_TITLE
    ...
    JOIN
         DEPARTMENT ON (DEPT_CODE = DEPT_ID);
  --EMPLOYEE테이블에서 DEPT_CODE가 NULL인 두 명의 사원은 조회X
  --DEPARTMENT 테이블에서 부서에 배정된 사원이 없는부서(D3, D4, D7)조회 X
  ...
  SELECT
   ...
   WHERE
         DEPT_CODE = DEPT_ID(+);
  -- 기준으로 삼지않을 테이블의 컬럼에 (+)를 붙여준다.
  
  --RIGHT [ OUTER ] JOIN : 두 테이블중 오른편에 기술한 테이블을 기준으로 JOIN
  -- 일치하는 컬럼이 존재하지않더라도 오른쪽 테이블의 데이터는 무조건 다 조회
 ...
 SELECT
  ...
  -- 3) FULL [ OUTER ] JOIN : 두 테이블 가진 모든 행을 조회할 수있는 조인
 SELECT
        EMP_NAME
      , DEPT_TITLE
   FROM
   ...
   OUTER
    JOIN
         DEARTMENT ON (DEPT_CODE = DEPT_ID);
  
  --> ORACLE : 오라클구문에서는 사용이 불가능
   ...
   * 모든 테이블의 각 행들을서로서로 매핑해서 조회된(공지합) **사용금지 문법
   * 
   * 두 테이블의 행들이 고해진 조합 봅아냄 => 데이터가 많을수록 방대한 행이 생겨남
   * => 과부하의 위험
   */
 ...
 --------------------------------------------------------------------------------
   ...
   * '='를 사용하지 않는 조인
   * 
   * 컬럼값을 비교하는 경우가 아니라 "범위"에 포함되는 내요을 매칭
   * 
   * 
   */
  --EMPOYEE 테이블로부터 사원명, 
  ...
  SELECT EMP_NAME
    ...
    FROM
   	   ...
   	   , SAL_GRADE
   WHERE
         WHERE SALARY MIN_SAL AND MAX_SAL;
  ...
  -- 사수사번, 사수명, 사수폰번호
  SELECT 
         E.EMP_NAME, E.EMP_NAME, E.PHONE, E.MANAGER_ID 
  ...
  --ANSI
   SELECT
     ...
     LEFT
     JOIN
          EMPLOYEE , ON (E.MANAGER_ID = M.EMP_ID); 
 ------------------------------------------------------------------------------
/*
 * < 다중 JOIN >
 * 
 */
 -- 사원명 + 부서명 + 직급명 + 지역명(LOCAL_NAME)
  ...
  --EMP_NAME, DEPT_TITLE, JOB_NAMEM LOCAL_NAME
  -->SET구문
  SELECT
         EMP_NAME
       , DEPT_TITLE
  ...
  ---------------------------------------------------------------------
  /*
   * < 집합 연산자 SET OPERRATOR > 
   * 
   * 여러 개의 쿼리문을 가지고 하나의 쿼리문으로 만드는 연산자
   * 
   * - UNUION : 합집합( 두 쿼리문의 수행결과값을 더한 후 중복되는 부분을 제거)
   * - INSERTSECT : 교집합( 두 쿼리문의 수행 결과갑 중 중ㅂ고된 부분)
   * - MINUS  : 차집합( 선행 ㅜ커리문 결과값 빼기 후행 쿼리문의 결과값을 한 결과)
   * 
   * - UNION ALL : 합집합의 결과에 교집합을 더한 개념
   * (두 쿼리문을 수행한 결과값을 무조건 더함. 합집합에서 중복 제거를 하지않는 것)
   * =>중복행이 여러 번 조회 될 수 있음
   */
  ...
  SELECT
   ...
   WHERE
         SALARY > 3000000;
  --UNION쓰려면 SELECT절에 기숧하는 컬럼이 같아야함
  ...
  SELECT 
    ...
    FROM
         EMPLOYEE
   WHERE 
         DEPT_CODE='D5' AND SALARY >30000000;
